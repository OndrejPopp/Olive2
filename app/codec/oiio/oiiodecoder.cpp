...
TexturePtr OIIODecoder::RetrieveVideoInternal(const RetrieveVideoParams &p)
{
  ...
  if (!buffer_.is_allocated()
    ...
    if (p.divider == 1) {
      ...
      image_->read_image(image_->current_subimage(), image_->current_miplevel(), 0, -1,
       oiio_pix_fmt_, buffer_.data(), OIIO::AutoStride, buffer_.linesize_bytes());
    } else {
      OIIO::ImageBuf buf(image_->spec());
      image_->read_image(image_->current_subimage(), image_->current_miplevel(), 0, -1,
       image_->spec().format, buf.localpixels(), buf.pixel_stride(), buf.scanline_stride(), buf.z_stride());
